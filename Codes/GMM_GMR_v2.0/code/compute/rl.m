...
%         joint_mu_matrix(5, i) = Mu(1+DoFs, i) - gap;
        joint_mu_matrix(1, i) = Mu(1+DoFs, i);
        joint_mu_matrix(2, i) = Mu(1+DoFs, i) + gap;
        joint_mu_matrix(3, i) = Mu(1+DoFs, i) - gap;
    end
    ...
    for episode = 1 : 10000
        ...
        for step = 1 : 200
            ...
            % observe reward
            [rwd, h] = reward(step, Mu, DoFs, s_new, joint_mu_matrix, Jacobian, goal);
            %fprintf('rwd %f\n', rwd);
...
function x = decode(a, nbAct, nbState)
    x = zeros(1, nbState);
    a = a - 1;
    for i = nbState : -1 :1
        x(i) = mod(a, nbAct) + 1;
...
function [r, hand] = reward(queryTime, Mu, DoFs, s, joint_mu_matrix, Jacobian, goal) 
	%fprintf('queryTime %d, goal %f\n', queryTime, goal(queryTime));
	nbState = size(Mu, 2);
    Mu_new = Mu;
    for i = 1 : nbState
   ...
   % pause;
	...
	elseif (queryTime < 50 || queryTime >= 150)
		if hand(3) < 0.33
			r = -1;
			return;
		end
	else
		if abs(hand(3) - goal(queryTime)) > 0.005
			r = -1;
			return;
		end
    end
